...
_REQUIRED_FIELDS = frozenset(
    ("transaction_date", "effective_date", "debit", "credit", "running_balance")
)
_DATE_FORMATS = (
    "%d/%m/%Y",
    "%d-%m-%Y",
...
class SHBStatementAdapter:
    """Parse SHB workbooks into normalized statement rows."""
                ...
                rows.append(
                    NormalizedBankStatementRow(
                        ...
                        transaction_date=_statement_date(
                            transaction_value, row_number, "system date"
                        ),
                        effective_date=_statement_date(
                            _cell(values, columns["effective_date"]),
                            row_number,
                            "effective date",
                        ),
...
def _find_header(rows: Iterable[tuple[object, ...]]) -> tuple[int, dict[str, int]]:
    for row_number, values in enumerate(rows, start=1):
        ...
        if row_number >= 50:
            break
    raise ValueError("SHB statement headers not found")
