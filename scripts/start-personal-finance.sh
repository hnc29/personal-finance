...
build_web(){ (cd "$WEB_DIR" && rm -rf .next && NEXT_PUBLIC_API_URL="$API_URL" npm run build && pf_write_web_build_metadata "$WEB_DIR" "$API_URL"); }
...
pf_web_build_matches "$WEB_DIR" "$API_URL" || build_web >>"$WEB_LOG" 2>&1
...
if [[ "$web_ready" != 1 ]]; then
  ...
  build_web >>"$WEB_LOG" 2>&1 || { tail -40 "$WEB_LOG"; exit 1; }
  (cd "$WEB_DIR" && NEXT_PUBLIC_API_URL="$API_URL" exec npm run start -- --hostname 127.0.0.1 --port "$WEB_PORT") >>"$WEB_LOG" 2>&1 & WEB_PID=$!
